...
def calculate_licenses_expiration_dates():
    result = [str(lice) for lice in DrivingLicense.objects.order_by("-license_number").all()]
    return "\n".join(result)


def get_drivers_with_expired_licenses(due_date: date):
    issue_cut_off_date = due_date - timedelta(365)

    drivers_with_expired_licenses = Driver.objects.filter(
        license__issue_date__gt=issue_cut_off_date
    )
